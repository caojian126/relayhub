# 部署指南

本文面向第一次部署 RelayHub 的人，从零到能跑通大约 10 分钟。

当前版本：**v0.5.0**

---

## 目录

1. [准备工作](#1-准备工作)
2. [Zeabur 部署（推荐）](#2-zeabur-部署推荐)
3. [首次登录与初始化](#3-首次登录与初始化)
4. [接入中转站](#4-接入中转站)
5. [同步模型](#5-同步模型)
6. [建统一模型（推荐）](#6-建统一模型推荐)
7. [排顺序、设次数（「顺序」页）](#7-排顺序设次数顺序页)
8. [创建 API Key](#8-创建-api-key)
9. [客户端怎么填](#9-客户端怎么填)
10. [开启缓存](#10-开启缓存)
11. [日常运维](#11-日常运维)
12. [故障排查](#12-故障排查)
13. [本地 / 自建 Docker 部署](#13-本地--自建-docker-部署)

---

## 1. 准备工作

先确认三件事：

| 准备项 | 说明 |
|---|---|
| 一个 Zeabur 账号 | 已经能创建项目的就可以 |
| 若干中转站的 Base URL + API Key | 填进面板用，不会进环境变量 |
| 想好一个面板密码 | 这个会放环境变量，忘了也能找回来 |

---

## 2. Zeabur 部署（推荐）

### 2.1 把仓库 Fork 到自己账号

打开 https://github.com/caojian126/relayhub ，右上角 **Fork**。

> 不 Fork 也行，直接部署原仓库；但 Fork 一份方便你自己改。

### 2.2 新建项目

Zeabur 控制台 → **New Project** → 选一个区域 → **Deploy from GitHub** → 选中刚才的仓库。

Zeabur 会自动识别仓库根目录的 `Dockerfile`，不需要手动指定构建方式。

### 2.3 ⚠️ 挂载持久卷（最关键的一步）

服务页面 → **Storage / 存储** → **添加 Volume**：

| 项 | 填什么 |
|---|---|
| Mount Path（挂载路径） | **`/data`** |
| 容量 | 1 GB 就够 |

**只挂这一个路径，不要挂 `/app` 之类的地方。**

挂载点里会存两样东西：

- `relayhub.db` —— 站点、API Key、路由、日志、缓存
- `secret_key` —— 自动生成的登录态密钥（若未设 `SECRET_KEY`）

> 不挂卷的后果：每次重新部署，站点配置、Key、日志、缓存全部清空。

### 2.4 环境变量

服务页面 → **Environment Variables / 环境变量**：

| 变量 | 必填 | 填什么 |
|---|---|---|
| `ADMIN_PASSWORD` | **强烈建议** | 你的面板登录密码 |
| `ADMIN_USERNAME` | 否 | 面板账号，默认 `admin` |
| `SECRET_KEY` | 建议 | 随便一串长随机字符，如 64 位十六进制 |
| `TZ` | 建议 | `Asia/Shanghai` |
| `DATA_DIR` | 否 | 默认已经是 `/data` |
| `REQUEST_TIMEOUT` | 否 | 默认 `600` 秒，中转站排队慢的话可以调大 |

**设计原则：**

- **面板密码放环境变量** —— 忘记密码时在部署平台改一下、重启就能找回，不会被锁在面板外面
- **中转站 API Key 不放环境变量** —— 它们在面板里添加，存在持久卷的数据库里

`ADMIN_PASSWORD` 是**最高优先级**：只要它非空，每次启动都会把数据库里的密码校正成它。

### 2.5 开启公网访问

服务页面 → **Networking / 网络** → **Generate Domain**（生成域名）。

端口填 **8080**（容器内应用监听的端口）。Zeabur 会注入 `PORT` 环境变量，应用会自动适配，你不需要手动设 `PORT`。

部署完成后访问 `https://你生成的域名/healthz`，返回下面这样就说服务活了：

```json
{"ok": true, "version": "0.5.0", "time": 1757500000.0, "data_dir": "/data", "persistent": true}
```

`persistent` 是 `true`，说明检测到 `/data` 是一个独立挂载点，也就是持久卷挂对了。

### 2.6 验证持久卷真的生效（必做，1 分钟）

不要假设「挂上了就是对的」。按下面走一遍：

1. 打开 `https://你的域名/admin` 并登录
2. 随便加一个站点（名字填 `测试` 就行），保存
3. 服务页面 → **Redeploy / 重新部署**，等它重启完
4. 刷新面板 —— **`测试` 这个站点应该还在**

在，说明持久卷生效了。以后**重新部署、重新构建、服务重启、容器重建都不会丢配置**，不用重新填 API、重新拉模型、重新建统一模型。

不在，就是下面几种原因：

| 现象 | 原因 |
|---|---|
| 面板顶部出现黄色横幅「数据可能保不住」 | `/data` 没挂上，数据写在容器临时层里了 |
| `healthz` 里 `persistent` 是 `false` | 同上 |
| 卷看起来挂着，数据还是没了 | Mount Path 填错（比如填成 `/app/data`），或 `DATA_DIR` 被改成了别的路径 |

> `DATA_DIR` 和挂载路径必须指向**同一个目录**。默认值就是 `/data`，不设也行；一旦要改，两边一起改。

### 2.7 以后怎么更新

推代码到 GitHub 的 `main` 分支 → Zeabur 默认会自动重新部署。
不想自动的话，在服务设置里关掉 auto-deploy，改成手动 Redeploy。

更新不会丢数据 —— 数据库表结构是**自动迁移**的，新版本启动时会给旧库补上缺的表和列。
**不要删 `app.db` / `relayhub.db` 来升级**，那是升级方式里最差的一种，数据比代码重要。

---

## 3. 首次登录与初始化

打开 `https://你的域名/admin`。

- 如果你设了 `ADMIN_PASSWORD`，用 `ADMIN_USERNAME`（默认 `admin`）+ 那个密码登录
- 如果没设，密码是随机生成的，去 **服务 → 日志** 里找：

```
================================================================
[RelayHub] 已创建面板账号: admin
[RelayHub] 随机初始密码: xxxxxxxxxxxxxx
[RelayHub] 建议设置环境变量 ADMIN_PASSWORD，以免忘记后进不去
================================================================
```

---

## 4. 接入中转站

左侧切到 **「站点」** 页，填：

| 字段 | 说明 |
|---|---|
| 名称 | 自己看得懂就行，如 `站A` |
| Base URL | `https://api.example.com` 或 `https://api.example.com/v1` **都行**，程序会自动补 `/v1` |
| API Key | 中转站给你的 `sk-...` |
| 启用 | 是 |

> 站点的**调用顺序不在这里设** —— 去「顺序」页拖（见第 7 节）。
> 早期版本有个「优先级」数字框，已经拿掉了：填数字既绕又容易和节点顺序打架。

填完点 **保存**，然后点这一行的 **「拉模型」**：

- 成功 → 会告诉你发现了几个模型，可以一键全部导入路由
- 失败 → 把提示信息（HTTP 状态码 + 返回内容）贴出来排查

站点列表里还能：

- **查额度** —— 立即拉一次剩余额度（支持 new-api / one-api 系）。
  会弹窗列出**取值字段、说明、错误和站点原始返回**，对不上时一眼能看出
- **解熔断** —— 站点被熔断后手动恢复
- **编辑** —— 改地址、换 Key（Key 留空表示不修改）

---

## 5. 同步模型

左侧切到 **「模型」** 页，点 **「扫描全部站点」**。

弹层里会按站点分组列出所有拉到的模型：

- 已导入的会置灰并标「已导入」，**不会覆盖你已经配置过的上游名和限额**
- 未导入的默认勾选
- 底部可以设 **默认每日上限**，导入时一次性套用

点「导入选中的 N 个」完成。

### 关于每日限额

有两个口径，可以叠加：

**1. 整站总次数**（「顺序」页）—— 该站**所有**统一模型累加

想表达「这个站一天最多用 100 次，不管问的是哪个模型」，就用它：

| 字段 | 说明 |
|---|---|
| 留空 / `0` | 不限制 |
| 大于 `0` | 该站点**一天总共**最多被调用这么多次 |

页面右侧有进度条显示「今天已用 / 上限」，用满自动跳过该站，次日按 `TZ` 重置。

**2. 站点 × 模型**（路由列表）—— 只算这一个模型在该站的用量

- `0` = 不限制
- 大于 `0` = 该站点在**该模型**上每天最多被调用这么多次

**计数规则：每次向上游发出请求都计一次，包含失败的请求。** 这样即使某个站持续故障，也不会反复消耗它的额度。次日 0 点（按 `TZ`）重置。

### 关于负载均衡

同一个模型名导入到多个站点 → 请求会自动在这些站点之间分配。调度方式在「设置」页或「顺序」页选：

| 方式 | 行为 |
|---|---|
| `priority` | **严格按你排的顺序**，第一个可用的就一直用（推荐） |
| `round_robin` | 按当日已用次数轮流，谁今天用得少先给谁 |
| `balanced` | 额度已知的站点中，余额多的优先 |

> ⚠️ **`balanced` 下你排的顺序基本不起作用** —— 它先看余额，顺序只在余额相同时才参与比较。
> 拖完顺序发现「没生效」，先看是不是 `balanced`。

---

## 6. 建统一模型（推荐）

这是 RelayHub 最有用的功能，强烈建议不要跳过。

### 为什么需要

不同中转站经常给同一个模型起不同名字：

| 站点 | 它那边的模型名 |
|---|---|
| A站 | `claude-sonnet-4-5` |
| B站 | `claude-4.5-sonnet` |
| C站 | `claude-sonnet-4.5-20250929` |

**RelayHub 不会用字符串相似度去猜它们是不是同一个模型** —— 猜错了会把请求打到错的模型上，比慢一点严重得多。它让你手动绑。

### 怎么建

左侧切到 **「统一模型」** 页 → 右上角 **「新建统一模型」**。

填一个客户端用的名字，比如 `auto`：

- **统一模型名**：`auto`（客户端就请求这个名字）
- **策略**：留空即可（跟随全局调度方式）。想让这个模型单独一套，再选 `priority`
- **对外可见**：选「是」，它才会出现在 `/v1/models` 里

建好后点 **「管理节点」**，把各站的真实模型加进去：

1. 选 **站点**（比如 A站）
2. 点 **「拉取该站模型名」**，输入框变成可下拉的候选列表
3. 选一个真实模型名 → **「添加节点」**
4. 换下一个站点重复，直到加完

```
auto
 ├ 1. A站 / claude-sonnet-4-5
 ├ 2. B站 / claude-4.5-sonnet
 └ 3. C站 / claude-sonnet-4.5-20250929
```

### 调整顺序

**这里拖的是「这个统一模型自己」的节点顺序**，松手自动保存。

> 默认情况下**不用在这里拖** —— 所有统一模型都跟随「顺序」页排的全局站序。
> 一旦你在这里拖了，这个模型就改听自己的，列表里会标成「自己一套顺序」，
> 随时可以点「恢复跟随全局站序」退回来。

每个节点右边实时显示状态：

| 状态 | 含义 |
|---|---|
| 正常 | 可用 |
| 最近失败 | 上次请求出错了，下次仍会尝试它 |
| 冷却中 Ns | 连续失败太多次，暂时跳过，倒计时结束自动恢复 |

「成功/失败」是累计次数，「最近错误」是最后一次的原文，排错直接看这一列。

### 节点和站点是两层冷却

- **节点级**：某个「站点 × 模型」连续失败 → 只有这个节点冷却，同一站的**其它模型不受影响**
- **站点级**：整个站点连不上 → 该站所有模型一起冷却

阈值和时长都在「设置」页调。到点自动恢复，**不会永久拉黑偶发故障的节点**。

### 同一个真实模型可以给多个统一模型用

比如再建一个 `cheap`，把各站的便宜模型绑进去。两者互不影响。

---

## 7. 排顺序、设次数（「顺序」页）

这一页就干两件事：**谁先用**、**每个站一天能用几次**。

```
≡  1  A站     ● 正常     12 / 100  [████░░░░]        [ 100 ]
≡  2  B站     ● 正常      3 / 50   [█░░░░░░░]        [  50 ]
≡  3  C站     ⏸ 停用      0 / 不限                    [     ]
```

### 排顺序

**两种排法都行**，点完/松手立刻保存，没有「保存」按钮：

- **点右边的 `↑` `↓`** —— 跟相邻的站换一位。**手机推荐用这个**
  （长按拖动容易触发手机系统的拖拽手势，比如「任意门」）
- **按住左边的 `≡` 上下拖动** —— 一次挪多位，桌面端更顺手

```
≡  1  A站   ● 正常   12/100  [███░░]  [ 100 ]  [↑] [↓]
≡  2  B站   ● 正常    3/50   [█░░░░░]  [  50 ]  [↑] [↓]
≡  3  C站   ⏸ 停用    0/不限            [     ]  [↑] [↓]
```

越靠上越优先。这个顺序是**全局**的：所有统一模型都按它挑站。默认不需要去「统一模型」页单独排序。

> 想让某个统一模型例外（比如它就是要固定先走 C 站），在那个模型的「管理节点」里拖一次即可，
> 它会标成「自己一套顺序」，不再跟随全局。想退回，点「恢复跟随全局站序」。

### 设「一天总共能用几次」

在每行右边填数字，**留空 = 不限**。这是**整站口径** ——
跨该站所有统一模型累加。比如填 `100`，就是「这个站今天不管问什么模型，加起来最多 100 次」。

用满了会自动跳过该站，次日 0 点（按 `TZ` 时区）重置。

### 选调度方式

页面顶部三个选项，用大白话写的：

| 选项 | 对应 | 行为 |
|---|---|---|
| **按我排的顺序** | `priority` | 严格从上往下试，第一个能用的就一直用 |
| **轮流用** | `round_robin` | 谁今天用得少先给谁 |
| **谁剩余额度多用谁** | `balanced` | 余额多的优先 |

> ⚠️ **选「谁剩余额度多用谁」时，你排的顺序基本不起作用** —— 它先看余额，顺序只在余额相同时才参与比较。
> 这是最容易踩的坑：拖完顺序发现没生效，先看这里。
> 页面会直接提示当前处于哪种状态，并给一个一键切换的链接。

---

## 8. 创建 API Key

左侧切到 **「密钥」** 页。

> ⚠️ **如果你还没有创建任何 Key，接口处于「开放模式」——任何人都能调用你的接口。** 面板顶部会有红字提醒。

点「生成」创建一个（可以给每个设备/每个人单独建一个，各自统计）。

生成的 Key 形如 `sk-rh-xxxxxxxx`。

同一页上方有 **「接口地址」** 卡片，列出四个入口的完整 URL，可以直接对照填写。

---

## 9. 客户端怎么填

四个入口共享同一套中转站、路由、限额和缓存。按你的客户端类型选一个：

| 协议 | 方法 | 路径 | 客户端 Base URL 填 |
|---|---|---|---|
| OpenAI | POST | `/v1/chat/completions` | `https://你的域名/v1` |
| OpenAI Responses | POST | `/v1/responses` | `https://你的域名/v1` |
| Anthropic | POST | `/v1/messages` | `https://你的域名` |
| Gemini | POST | `/v1beta/models/{模型}:generateContent` | `https://你的域名` |

认证统一用：

```
Authorization: Bearer sk-rh-你的密钥
```

Gemini 客户端也支持把 Key 放在 URL 上：`?key=sk-rh-你的密钥`。

**模型名填统一模型名**（比如 `auto`），不是上游的真实模型名 —— 真实模型名只用于后台路由，不会暴露给客户端。想确认有哪些可用，调一次 `GET /v1/models`。

### 自测命令

**OpenAI 格式（流式）**

```bash
curl -N https://你的域名/v1/chat/completions \
  -H "Authorization: Bearer sk-rh-你的密钥" \
  -H "Content-Type: application/json" \
  -d '{"model":"gemini-2.5-pro","messages":[{"role":"user","content":"你好"}],"stream":true}'
```

**Anthropic 格式**

```bash
curl -N https://你的域名/v1/messages \
  -H "x-api-key: sk-rh-你的密钥" \
  -H "anthropic-version: 2023-06-01" \
  -H "Content-Type: application/json" \
  -d '{"model":"gemini-2.5-pro","max_tokens":256,"messages":[{"role":"user","content":"你好"}]}'
```

> Anthropic 客户端把它自己的 Key 放在 `x-api-key` 头里，RelayHub 会同时认 `Authorization: Bearer` 和 `x-api-key` 两种写法。若你的客户端把 Key 写死在 `x-api-key`，直接把 `sk-rh-...` 填进去即可。

**Gemini 格式**

```bash
curl -N "https://你的域名/v1beta/models/gemini-2.5-pro:generateContent?key=sk-rh-你的密钥" \
  -H "Content-Type: application/json" \
  -d '{"contents":[{"role":"user","parts":[{"text":"你好"}]}]}'
```

流式把 `:generateContent` 换成 `:streamGenerateContent`。

**Responses 格式**

```bash
curl -N https://你的域名/v1/responses \
  -H "Authorization: Bearer sk-rh-你的密钥" \
  -H "Content-Type: application/json" \
  -d '{"model":"gemini-2.5-pro","input":"你好","stream":true}'
```

---

## 10. 开启缓存

**缓存默认是关的**，需要在「设置」页手动开。

### 原理

```
OpenAI / Responses / Anthropic / Gemini   四种客户端格式
        ↓ 翻译成统一的 OpenAI 请求体
     【 缓 存 】  key = 统一请求体的哈希
        ↓ 未命中才打上游
      中转站
        ↓ 翻译回客户端的格式
```

**缓存在「统一请求体」这一层，所以四种格式共享同一份缓存。** 用一个格式问过的问题，换另一种格式再问也能命中。

### 设置项

| 设置 | 说明 | 建议 |
|---|---|---|
| 缓存开关 | 总开关 | 开 |
| 仅缓存 `temperature=0` | 只缓存确定性请求，避免返回错误答案 | **开**（推荐） |
| 有效期 | 秒，`0` = 永不过期 | 3600 |
| 最大条目数 | 超出后按最近最少使用淘汰 | 1000 |
| 允许读写缓存的入口 | 不勾的入口完全绕过缓存 | 按需，至少勾 OpenAI |

### 注意

- 如果客户端不传 `temperature`，默认是 1（有随机性），此时**不会**被缓存。想让缓存生效，客户端要把温度设为 0
- 关了「仅缓存 temperature=0」后，只要请求体完全一致就命中——**上下文不同的对话可能拿到相同回复**，谨慎使用
- 命中缓存不消耗中转站额度，也不计入站点每日限额
- 概览页和日志里都能看到命中情况（日志的「来源」列会显示「缓存」）

---

## 11. 日常运维

### 概览页

今日请求数、成功率、缓存命中率、缓存条目数、Token 用量、近 7 天柱状图、站点状态一览。

### 日志页

每次请求（含失败和重试）都会记录，可以看到：入口格式、实际打到了哪个站点（或命中缓存）、用了哪个 Key、第几次尝试、耗时、Token、错误内容。支持按站点筛选和自动刷新。

### 自动同步模型

**默认关闭。** 需要时在「设置」页打开，可设间隔（默认 24 小时一次）。

> ⚠️ 打开后它会把上游**所有**模型自动导进路由。上游动辄几百个模型，
> 所以默认关着 —— 想用哪个模型，去「模型」页按站点勾选导入，可控得多。

打开后也**只往路由里新增上游新出现的模型，绝不删你配过的东西**。

### 额度巡检

定时调用各站的额度接口查询剩余额度。**`/v1/dashboard/billing/*` 和 `/dashboard/billing/*` 两个前缀都会试** —— 老站常常只挂裸路径。部分站点两个都关闭，此时显示「未知」，**不影响路由**。

> **显示成很大的数（比如 1 亿）是怎么回事？**
>
> 那是上游表示**「不限额度」的占位值**，不是真实余额。one-api / new-api 系的分叉普遍把「不限」
> 写成 `hard_limit_usd = 100000000`。RelayHub 会把 `>= 1 亿` 或负数的值认定为「不限」，
> 不当成余额显示、也不参与 `balanced` 排序，并在面板上注明是占位值。
>
> 想确认真实结构，在站点列表点 **「查额度」**，弹窗里有取值字段、说明和**站点原始返回**。

### 备份

设置页 → **导出配置**，会下载包含站点、Key、路由、设置的 JSON。换服务器时用 **导入配置** 恢复。

> 导出的 JSON 包含明文密钥，请妥善保管。

---

## 12. 故障排查

### 面板打不开 / 502

1. 先访问 `/healthz`，能返回 JSON 说明应用是活的，问题在网络或域名
2. 检查服务的公网端口是否设成了 `8080`
3. 看服务日志有没有启动报错

### 登录不上

- 设了 `ADMIN_PASSWORD` 却登不上 → 环境变量改完要**重启服务**才生效
- 没设 → 去服务日志里找随机密码
- 实在不行 → 设一个新的 `ADMIN_PASSWORD` 并重启，密码会被强制校正

### 调用返回 401

```
{"detail":"缺少 API Key"}  → 没带 Authorization 头
{"detail":"API Key 无效或已禁用"} → Key 填错了，或者被你在面板里停用了
```

### 调用返回 503

```
没有可用上游支持模型 xxx
```

模型名没配到路由里，或者配了但站点全被停用/熔断/达到每日上限。去「模型」页确认这个名字存在。

### 调用返回 502

```
所有上游均失败。最后错误：...
```

后面会带上每个站的具体错误。常见原因：

- 中转站的 Key 过期或余额耗尽（返回 401 / 402 / 403）
- 该站不支持你请求的这个模型（返回 404 / 400）
- 该站被限流（返回 429）

去「日志」页能看到每一次尝试的完整错误。

### 某个站点连续失败后进入冷却

那是熔断：连续失败达到阈值（默认 3 次）后会冷却一段时间（默认 600 秒），期间不参与路由，到点自动恢复。可以在「站点」页点 **解熔断** 立即恢复，或者在设置页调整阈值和冷却时间。

### 缓存不生效

按顺序检查：

1. 设置页的「缓存开关」开了吗
2. 当前入口在这个「允许读写缓存的入口」名单里吗
3. 请求里的 `temperature` 是 `0` 吗（开了「仅缓存 temperature=0」时）
4. 请求体是否完全一致（系统提示词、历史消息、参数任何一个不同都算不同请求）
5. 看日志的「来源」列，命中时会显示「缓存」标签

### 流式请求中途断了

一旦上游开始返回内容，就不能再切换到其他站点（客户端已经收到一半内容了）。切换只发生在「请求前」和「还没返回第一个字节」时。已经吐字后中断会记一次失败并计入熔断。

---

## 13. 本地 / 自建 Docker 部署

### Docker Compose

```yaml
services:
  relayhub:
    build: https://github.com/caojian126/relayhub.git
    container_name: relayhub
    restart: unless-stopped
    ports:
      - "8080:8080"
    environment:
      - ADMIN_USERNAME=admin
      - ADMIN_PASSWORD=换成一个强密码
      - SECRET_KEY=换成一串长随机字符
      - TZ=Asia/Shanghai
      - DATA_DIR=/data
    volumes:
      - ./data:/data
```

```bash
docker compose up -d
```

访问 http://localhost:8080/admin

### 直接跑 Python

```bash
pip install -r requirements.txt

DATA_DIR=./data \
ADMIN_PASSWORD=admin123 \
uvicorn app.main:app --host 0.0.0.0 --port 8080
```

### 反向代理注意事项

如果用 Nginx / Caddy 在前面，务必**关闭对 SSE 的缓冲**，否则流式会变成一次性输出：

Nginx：

```nginx
location / {
    proxy_pass http://127.0.0.1:8080;
    proxy_http_version 1.1;
    proxy_set_header Host $host;
    proxy_set_header X-Real-IP $remote_addr;
    proxy_buffering off;
    proxy_cache off;
    proxy_read_timeout 600s;
    chunked_transfer_encoding on;
}
```

Caddy 默认不缓冲，一般无需额外配置。

---

## 附：环境变量总表

| 变量 | 必填 | 默认值 | 说明 |
|---|---|---|---|
| `ADMIN_PASSWORD` | 强烈建议 | 随机生成 | 面板密码，最高优先级，忘了就改它 |
| `ADMIN_USERNAME` | 否 | `admin` | 面板账号，不设则不干预面板里改过的账号 |
| `SECRET_KEY` | 建议 | 自动生成 | 登录态签名密钥，不设则随机生成并存在卷里 |
| `TZ` | 建议 | `Asia/Shanghai` | 影响每日限额重置时间与图表按天切分 |
| `DATA_DIR` | 否 | `/data` | 数据目录，Dockerfile 里已经写死 |
| `DB_PATH` | 否 | `$DATA_DIR/relayhub.db` | 数据库路径 |
| `CONNECT_TIMEOUT` | 否 | `15` | 连接上游超时（秒） |
| `REQUEST_TIMEOUT` | 否 | `600` | 上游整体请求超时（秒） |
| `PORT` | 否 | `8080` | 监听端口，Zeabur 会自动注入 |
