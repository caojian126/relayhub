...
| **站点调用顺序** | 「顺序」页点 `↑` `↓` 或拖拽排各站的先后，全局生效，点完即存。想让某个统一模型例外，在那个模型里单独拖一次即可 |
| 三种调度方式 | `priority` 严格按你排的顺序 · `round_robin` 谁今天用得少给谁 · `balanced` 谁剩余额度多用谁 |
...
| **整站每日总次数** | 「顺序」页给每个站设「一天总共能调几次」，跨该站所有统一模型累加。留空 = 不限 |
| 节点级每日限额 | 也可按「站点 x 模型」单独设上限，用于限制某个模型在某个站的用量 |
| 模型自动同步 | 直接从各上游拉取 `/v1/models`，勾选即可导入，支持定时自动同步 |
| 额度巡检 | 定时查询各站剩余额度（new-api / one-api 系通用），自动识别「不限」占位值 |
...
| 管理面板 | 单文件 SPA，零构建、无外部依赖；内置聊天测试区，直接打本机网关。共 9 个页：概览 / 站点 / 模型 / 统一模型 / **顺序** / 测试 / 密钥 / 日志 / 设置 |
...
3. **建统一模型（推荐，这一步是关键）**
   ...
   **为什么需要它**：同一个模型在不同站点经常叫不同的名字
   ...
   > 默认情况下，所有统一模型都**跟随「顺序」页排的全局站序**。
   > 一旦你在某个统一模型里手动拖过节点，这个模型就改听自己的顺序，
   > 列表里会标成「自己一套顺序」，随时可以点「恢复跟随全局站序」退回来。

4. **排顺序、设次数**
   到「顺序」页：

   - 排顺序：点右边的 `↑` `↓` 与相邻的站换位（**手机推荐**，长按拖动容易触发系统手势），
     或者按住 `≡` 上下拖动。点完/松手即存
   - 在每行右边填**这个站一天总共能调几次**，留空 = 不限。
     这是整站口径，跨该站所有统一模型累加，用满自动跳过，次日按 `TZ` 重置

   如果还想限制得更细（比如「A 站的 gpt-5 每天最多 50 次」），
   可以另外在路由列表里按「站点 x 模型」设节点级上限。设为 `0` 表示不限制。
...
顺序在「顺序」页拖出来，三种调度方式决定它怎么被使用：

| 方式 | 行为 | 适用场景 |
|---|---|---|
| `priority` | **严格按你排的顺序**，第一个可用的持续使用 | 想让顺序说了算（推荐） |
| `round_robin` | 按当日已用次数轮流，谁今天用得少先给谁 | 各站额度都不大，希望均匀消耗 |
| `balanced` | 在额度已知的站点中，**余额多的优先**；额度未知的排在后面 | 站点较多且额度可查询 |

> ⚠️ **`balanced` 下你排的顺序基本不起作用** —— 它先看余额，顺序只在余额相同时才参与比较。
> 如果你拖完顺序发现「没生效」，先看这里是不是 `balanced`。
> 「顺序」页会直接提示当前处于哪种状态，并给一个一键切换的链接。

顺序有两层，默认只有一层：

1. **全局站序** —— 在「顺序」页拖出来的，所有统一模型共用
2. **模型自己的节点序** —— 只有在某个统一模型里手动拖过节点才会产生

一旦某模型有了自己的节点序，它就**不再跟随全局站序**，列表里标为「自己一套顺序」，
点「恢复跟随全局站序」可以退回来。
...
| 口径 | 在哪里设 | 计数方式 |
|---|---|---|
| **整站总次数** | 「顺序」页 | 该站**所有**统一模型累加。想「这个站一天最多用 100 次」就用它 |
| **站点 × 模型** | 路由列表 | 只算这一个模型在该站的用量 |

**每次向上游发出请求都计一次，包含失败的请求** —— 这样即使某个站点持续故障，也不会反复消耗它的额度。次日 0 点（按 `TZ` 时区）重置。两种口径都为 `0` 或留空表示不限制。
...
定时调用各站的 `/dashboard/billing/subscription` 与 `/dashboard/billing/usage`（new-api / one-api 系通用）。
**带 `/v1` 前缀和不带前缀的路径都会试** —— 老站常常只挂裸路径，以前只试 `/v1` 会误判成「未提供额度接口」。

> **关于「剩余额度」显示为一个很大的数（例如 1 亿）**
>
> 这是上游的**「不限额度」占位值**，不是真实余额。one-api / new-api 系的分叉普遍把
> 「不限」写成 `hard_limit_usd = 100000000` 这种大数。
> RelayHub 会把 `>= 1 亿` 或负数的值一律认定为「不限」：**不显示为余额，也不参与 `balanced` 排序**
> （否则所有站看起来一样富，排序会全乱），并在面板上注明「站点返回的是占位值」。
>
> 在站点列表点「查额度」会弹出详情，把**取值字段、说明、错误和站点原始返回**都列出来，对不上时一眼能看出。
